...
int OBJModel::LoadModelData(const char *path, const char* filename)
{
    _path = path;
    std::stringstream fn;
    fn << path << filename;
    std::ifstream in(fn.str().c_str());     //open the model file
    if(!in.is_open())
    {
    ...
    while(!in.eof())
    {
        std::string data;
        getline(in, data);
        if(data[0]=='#') //if it's a comment
            continue;       //we don't have to do anything with it
        else if(data[0]=='v' && data[1]==' ')     //if a vertex
        {
            float tmpx,tmpy,tmpz;
            sscanf(data.c_str(),"v %f %f %f",&tmpx,&tmpy,&tmpz);       //read the 3 floats, which makes up the vertex
            vertex.push_back(new coordinate(tmpx,tmpy,tmpz));       //and put it in the vertex vector
        } else if(data[0]=='v' && data[1]=='n')    //if it's a normal vector
        {
            float tmpx,tmpy,tmpz;
            sscanf(data.c_str(),"vn %f %f %f",&tmpx,&tmpy,&tmpz);
            normals.push_back(new coordinate(tmpx,tmpy,tmpz));      //basically do the same
            isnormals=true;
        } else if(data[0]=='f')   //if it's a face
        {
            int a,b,c,d,e;
            if(count(data.begin(),data.end(),' ')==4)     //if this is a quad
            {
                if(data.find("//")!=std::string::npos)     //if it's contain a normal vector, but not contain texture coorinate
                {
                    sscanf(data.c_str(),"f %d//%d %d//%d %d//%d %d//%d",&a,&b,&c,&b,&d,&b,&e,&b);      //read in this form
                ...
                } else if(data.find("/")!=std::string::npos)        //if we have texture coorinate and normal vectors
                {
                    int t[4];       //texture coorinates
                    //read in this form, and put to the end of the vector
                    sscanf(data.c_str(),"f %d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d",&a,&t[0],&b,&c,&t[1],&b,&d,&t[2],&b,&e,&t[3],&b);
                    faces.push_back(new face(b,a,c,d,e,t[0],t[1],t[2],t[3],curmat));
                } else {
                    //else we don't have normal vectors nor texture coorinate
                    sscanf(data.c_str(),"f %d %d %d %d",&a,&b,&c,&d);
                ...
                {
                    sscanf(data.c_str(),"f %d//%d %d//%d %d//%d",&a,&b,&c,&b,&d,&b);
                    faces.push_back(new face(b,a,c,d,0,0,0,curmat));
                } else if(data.find("/")!=std::string::npos)
                {
                    int t[3];
                    sscanf(data.c_str(),"f %d/%d/%d %d/%d/%d %d/%d/%d",&a,&t[0],&b,&c,&t[1],&b,&d,&t[2],&b);
                    faces.push_back(new face(b,a,c,d,t[0],t[1],t[2],curmat));
                } else {
                    sscanf(data.c_str(),"f %d %d %d",&a,&b,&c);
                    faces.push_back(new face(-1,a,b,c,0,0,0,curmat));
                }
            }
        } else if(data[0]=='u' && data[1]=='s' && data[2]=='e')     //use material material_name
        {
            char tmp[200];
            sscanf(data.c_str(),"usemtl %s",tmp);      //read the name of the material to tmp
            for(size_t i=0; i<materials.size(); i++)   //go through all of the materials
            {
        ...
        } else if(data[0]=='m' && data[1]=='t' && data[2]=='l' && data[3]=='l')      //material library, a file, which contain
            //all of the materials
        {
            char filen[200];
            sscanf(data.c_str(),"mtllib %s",filen);    //read the filename
            std::stringstream fn;
            fn << path << filen;
            std::ifstream mtlin(fn.str().c_str());     //open the file
            ...
            for(size_t i=0; i<tmp.size(); i++) //go through all lines of the mtllib file
            {
                ...
                } else if(tmp[i][0]=='m' && tmp[i][1]=='a')      //and the texture
                {
                    ...
                    texture=tload->Load2DMipmap(fn.str().c_str());  //read the filename, and use the loadTexture function to load it, and get the id.
                    if (texture == 0)
                    {
                        ismat=false;
                        texture = -1;
        ...
        } else if(data[0]=='v' && data[1]=='t')    //back to the obj file, texture coorinate
        {
            float u,v;
            sscanf(data.c_str(),"vt %f %f",&u,&v);     //read the uv coordinate
    ...
    for(size_t i=0; i<faces.size(); i++) //go throught all faces
    {
