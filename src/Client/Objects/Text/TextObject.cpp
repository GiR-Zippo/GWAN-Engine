...
TextObject::TextObject(uint16 id, float x, float y, float z, bool depth, float height, std::string text, std::string font):
...
void TextObject::Draw()
{
    ...
    if (_centerX && _centerY)
        print((float)glutGet(GLUT_WINDOW_WIDTH) /2 + _textlen/2, (float)glutGet(GLUT_WINDOW_HEIGHT)/2 + _height, _text.c_str());
    else if (_centerX)
        print((float)glutGet(GLUT_WINDOW_WIDTH) /2 + _textlen/2, _y * h, _text.c_str());
    else if (_centerY)
...
void TextObject::init(const char* fname)
{
...
void TextObject::make_dlist ( FT_Face face, char ch, GLuint list_base, GLuint* tex_base )
{
...
void TextObject::print(float x, float y, const char* fmt, ... )
{
    ...
    for (int i = 0; i < lines.size(); i++)
    {
        ...
        _textlen=x-rpos[0]; //(Assuming No Rotations Have Happend)
        glPopMatrix();
    }
...
bool TextObject::IsMouseHit(float x, float y)
{
    return false;
}