...
uint64 FileHandler::GetFileSize(std::string filename)
{
    uint64 size = 0;
#if WIN32
    _fmode = _O_BINARY;
...
std::map<uint8, unsigned char> FileHandler::GetMD5(std::string filename)
{
    ...
    ACE_HANDLE hfile = ACE_OS::open(iFilename.c_str(), O_RDONLY ,GENERIC_READ | FILE_FLAG_OVERLAPPED);
