#ifndef __SPLITSTRING_HPP__
#define __SPLITSTRING_HPP__
...
public:
  //! Create and initialize a new SplitString
  //!
  //! \param[in] src The string to split
  //! \param[in] delim The delimiter to split the string around
  SplitString (std::string& src, const std::string& delim, bool lower = false)
  {
    if (lower)
